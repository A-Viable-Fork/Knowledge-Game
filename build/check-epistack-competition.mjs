// Role: the oracle for the epistack-competition kernel, relocated from vendor/scaffolder/new-kernel.mjs's generated
...
// Contract: `node build/check-epistack-competition.mjs` exits non-zero on any failure.
...
const { KINDS, SOURCES, ADOPTED, ADOPTED_HASHES } = require("../communities/epistack-competition/corpus/tables.js");
const { COMMON_TYPE_HASHES } = require("../vendor/corpora/_shared/common-types.js");

let fails = 0;
const ok = (c, m) => { console.log(`${c ? "  ok  " : " FAIL "} ${m}`); if (!c) fails++; };
const H = "=".repeat(80);
console.log(H); console.log("CHECK-EPISTACK-COMPETITION: the founded epistack-competition kernel"); console.log(H);
...
let built = null;
try {
  const mod = await import("./epistack-competition-build.mjs");
...
console.log("\n" + H);
console.log(fails === 0 ? "check-epistack-competition: OK" : `check-epistack-competition: ${fails} FAILURE(S)`);
console.log(H);

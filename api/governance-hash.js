...
//   fields, the null-objective order, the corpus content license), the community's durable content-
//   addressed identity. kernel_id demotes to a human label over this hash, exactly as a kind name is a
//   label over a type-hash. Phase KG-10 moves this from build/governance-hash.mjs (which now re-
//   exports it) into api/ so the in-app kernel designer's finish screen can compute the identical
//   hash the founding flow's own card emit uses, from the same single implementation, never a second
...
//   imports only the vendored canonical form. Browser-safe ESM.
...
//   hash is permutation-invariant over field order. Phase KG-6 (market-layer absorption) adds
//   corpus_content_license: a community's declared terms (gift, attribution-required, share-alike,
//   non-commercial, or a named commercial term, per the market-layer document, upstream, branch-
//   local) are inside the hash so a fork carries them visibly, exactly as the argument requires;
//   absent, it is the honest null "unspecified", never silently omitted from the record.

...
//   Knowledge Game (Phase KG-6a). Constructs the project via @bubblewrap/core's library API directly
//   rather than its CLI's interactive prompts, so the exact same inputs run unattended in CI.
...
//   (packageId, launcher name, the committed test keystore's path and alias), and writes the full
//   Gradle project into this directory via TwaGenerator.createTwaProject, exactly as `bubblewrap
//   init` would, without the interactive prompts.

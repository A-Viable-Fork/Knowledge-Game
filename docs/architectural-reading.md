...
| The comment-support guard | `kernel/gate/comment-guard.mjs` | **pinned** | Landed at the `ce7ca28` re-pin (Phase KG-4) alongside the comment kind's shared-type bundle (`corpora/_shared/common-types.js`) and two additive link kinds (`kernel/schema/records.mjs`'s `comments-on`/`replies-to`). `rejectCommentSupport(contribution, storeView)` throws naming `comment-support-barred` if any `supports` link resolves its `from_identity` to kind `comment`; `local-provider.mjs`'s `propose()` calls it before `decide()`. This app calls the identical vendored function from `api/contribute.js` before its own `decide()` call, since this app's write path does not route through `local-provider.mjs`'s `propose()` (it calls `decide()` directly over its own store view), so the guard would otherwise not run on this app's own path. |
